...
//banner轮播图
	// $(function(){
		Zlunbo(".bannertu",".tongping",".bannertukuang ul li",["#8CC4F3","#E8E8E8","#69AC8A","#E8E8E8","#0EA9EF"],2,"#ececec",1000,1,"#A4AFD0",4)
	// })
...
//回到顶部
{
    let bns = document.querySelectorAll(".f1");
    let tips = document.querySelectorAll(".mui-mbar-tab-tip");
    bns.forEach(function (ele, index) {
        ele.onmouseover = function () {
            tips[index].style.display = "block";
            animate(tips[index], {left: -90, opacity: 1});
        }
        ele.onmouseout = function () {
            animate(tips[index], {left: -100, opacity: 0.3}, function () {
    ...
    window.onscroll=function(){
       for(let i=0;i<section.length;i++){
    	...
    	if(sobj.scrollTop>=600){
           ...
           btnBox.style.cssText="width:36px;height:324px;";
    	}else{
    		if(flags){
    			flags=false;
    			flagx=true;
    	  ...
    	  btnBox.style.cssText="width:0;height:0;";
    	 }
    }
